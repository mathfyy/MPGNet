...
def ev(output, y_true, compare_rule='>', thr=0.5):
    ...
    for c in range(0, n_classes):
        ...
        if pred_flat.sum() == 0 and true_flat.sum() == 0:
            ...
            # 增加评估指标
            GT_ture=true_flat.squeeze().astype(int)
            PRE_pred = pred_flat.squeeze().astype(int)
            S_meature=fowlkes_mallows_score(GT_ture.reshape((256*256*24)), PRE_pred.reshape((256*256*24)))
            F_measure = f1_score(GT_ture.reshape((256*256*24)), PRE_pred.reshape((256*256*24)))
            fai_gt = GT_ture.reshape((256, 256 * 24)) - GT_ture.reshape((256, 256 * 24)).mean() * np.ones(((256, 256 * 24)))
            fai_pr = PRE_pred.reshape((256, 256 * 24)) - PRE_pred.reshape((256, 256 * 24)).mean() * np.ones(((256, 256 * 24)))
            thegm=(2*fai_gt*fai_pr)/(fai_gt*fai_gt+fai_pr*fai_pr)
            thegm_f=(thegm+1)*(thegm+1)/4
            E_meature=thegm_f.sum()/(24*256*256)
            M_D=abs(PRE_pred-GT_ture).sum()/(24*256*256)

            #
            # fpr, tpr, thresholds = roc_curve(GT_ture.reshape(-1,1), PRE_pred.reshape(-1,1))
            # axes[int(c / 2), c % 2].plot(fpr, tpr, color=colors[c], marker=markers[c],
            #                              label="AUC: {:.2f}".format(auc(fpr, tpr)))
            # axes[int(c / 2), c % 2].set_xlabel("FPR")
            # axes[int(c / 2), c % 2].set_ylabel("TPR")
            # axes[int(c / 2), c % 2].legend(loc="lower right")
        ...
        dsc += dsc_t
        iou += iou_t
        S_meature_all += S_meature
        F_meature_all += F_measure
        E_meature_all += E_meature
        M_D_all += M_D

    return dsc/n_classes, iou/n_classes, specificity/n_classes, recall/n_classes, S_meature_all/n_classes, F_meature_all/n_classes, E_meature_all/n_classes, M_D_all/n_classes
