...
class EnDown(nn.Module):
    def __init__(self, in_channels, out_channels, pad=(1, 1, 1), stride=(2, 2, 2)):
        super(EnDown, self).__init__()
        self.conv = nn.Conv3d(in_channels, out_channels, kernel_size=3, stride=stride, padding=pad)
...
class Unet(nn.Module):
    def __init__(self, in_channels=4, base_channels=16, num_classes=4):
        ...
        self.EnDown3 = EnDown(in_channels=base_channels * 4, out_channels=base_channels * 8, pad=(1, 1, 1),
                              stride=(1, 2, 2))

        self.EnBlock4_1 = EnBlock(in_channels=base_channels * 8)
        self.EnBlock4_2 = EnBlock(in_channels=base_channels * 8)
        self.EnDown4 = EnDown(in_channels=base_channels * 8, out_channels=base_channels * 8, pad=(1, 1, 1),
                              stride=(1, 2, 2))

        self.EnBlock5_1 = EnBlock(in_channels=base_channels * 8)
        self.EnBlock5_2 = EnBlock(in_channels=base_channels * 8)
        self.EnBlock5_3 = EnBlock(in_channels=base_channels * 8)
        self.EnBlock5_4 = EnBlock(in_channels=base_channels * 8)
    ...
    def forward(self, x):
        ...
        x5_2 = self.EnBlock5_2(x5_1)
        x5_3 = self.EnBlock5_3(x5_2)
        output = self.EnBlock5_4(x5_3)  # (1, 128, 16, 16, 16)
...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        x = torch.rand((1, 4, 24, 256, 256), device=cuda0)
        model = Unet(in_channels=4, base_channels=4 * 3, num_classes=3)
        model.cuda()
        output = model(x)
