...
class MPEDANet(nn.Module):
    ...
    def forward(self, x):
        x_size=x.shape
        x = nn.functional.interpolate(x, scale_factor=(
        self.image_size[0] / x.shape[2], self.image_size[1] / x.shape[3], self.image_size[2] / x.shape[4]),
                                      mode='nearest')
        x = self.norm(x)
        x1 = self.in_conv1(x)
        ...
        x = nn.functional.interpolate(x, scale_factor=(x_size[2]/x.shape[2],  x_size[3]/x.shape[3], x_size[4]/x.shape[4]),
                                      mode='nearest')
...
if __name__ == '__main__':
    ...
    sizeWHD = [24, 256, 256]
    x = torch.randn([1, 4, sizeWHD[0], sizeWHD[1], sizeWHD[2]])
    net = MPEDANet(in_channels=4, num_classes=4)
    y = net(x)
    print("params: ", sum(p.numel() for p in net.parameters()))
    print(y.shape)
