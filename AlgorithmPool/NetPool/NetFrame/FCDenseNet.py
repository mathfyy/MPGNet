...
class DenseNet3D(nn.Module):
    def __init__(self, inC, outC, kerSize=(3, 3, 3), kerStride=(1, 1, 1), kerPad=(1, 1, 1), dilate=(1, 1, 1),
        ...
        self.out.add_module("conv", nn.Conv3d(5 * inC // 4, outC, kerSize, stride=kerStride, padding=kerPad, bias=True,
                             dilation=dilate))
        self.out.add_module("norm", normalization3D(outC, norm))
        self.out.add_module("relu", nn.LeakyReLU(inplace=True))
...
class FCDenseNet3D(nn.Module):
    def __init__(self, in_channels=1, out_channels=1):
        ...
        self.DenseNet3D2 = DenseNet3D(48, 112)
        ...
        self.DenseNet3D3 = DenseNet3D(56, 120)
        ...
        self.DenseNet3D4 = DenseNet3D(60, 124)
    ...
    def forward(self, input):
        ...
        pre3 = F.interpolate(self.LeakyReLU(self.bn3d_(self.DC3_(pre3))), scale_factor=(1, 2, 2), mode='trilinear',
                             align_corners=True)
        pre3 = self.FC3_(pre3)
        pre3 = F.interpolate(pre3, scale_factor=(1, 2, 2), mode='trilinear', align_corners=True)
        ...
        pre4 = F.interpolate(self.LeakyReLU(self.bn4d_(self.DC4_(pre4))), scale_factor=(1, 2, 2), mode='trilinear',
                             align_corners=True)
        pre4 = F.interpolate(self.LeakyReLU(self.bn4d__(self.DC4__(pre4))), scale_factor=(1, 2, 2), mode='trilinear',
                             align_corners=True)
        pre4 = self.FC4_(pre4)
        pre4 = F.interpolate(pre4, scale_factor=(1, 2, 2), mode='trilinear', align_corners=True)
...
class FCDenseNet3D_ncct(nn.Module):
    def __init__(self, in_channels=1, init_features=4, out_channels=1):
        super(FCDenseNet3D_ncct, self).__init__()
        self.n_features = init_features
        features = init_features

        self.conLayer1 = nn.Sequential()
        self.conLayer1.add_module("conv",
                                  nn.Conv3d(in_channels, features*2, 3, stride=(1, 2, 2), padding=1, bias=True))
        self.conLayer1.add_module("norm", normalization3D(features*2, "bn"))
        self.conLayer1.add_module("relu", nn.LeakyReLU(inplace=True))

        self.DenseNet3D1 = DenseNet3D(features*2, features*6)
        self.conLayer2 = nn.Sequential()
        self.conLayer2.add_module("conv",
                                  nn.Conv3d(features*6, features*3, 3, stride=(1, 2, 2), padding=1, bias=True))
        self.conLayer2.add_module("norm", normalization3D(features*3, "bn"))
        self.conLayer2.add_module("relu", nn.LeakyReLU(inplace=True))

        self.DenseNet3D2 = DenseNet3D(features*3, features*7)
        self.conLayer3 = nn.Sequential()
        self.conLayer3.add_module("conv",
                                  nn.Conv3d(features*7, features*4, 3, stride=(1, 2, 2), padding=1, bias=True))
        self.conLayer3.add_module("norm", normalization3D(features*4, "bn"))
        self.conLayer3.add_module("relu", nn.LeakyReLU(inplace=True))

        self.DenseNet3D3 = DenseNet3D(features*4, features*8)
        self.conLayer4 = nn.Sequential()
        self.conLayer4.add_module("conv",
                                  nn.Conv3d(features*8, features*4, 3, stride=(1, 2, 2), padding=1, bias=True))
        self.conLayer4.add_module("norm", normalization3D(features*4, "bn"))
        self.conLayer4.add_module("relu", nn.LeakyReLU(inplace=True))

        self.DenseNet3D4 = DenseNet3D(features*4, features*8)

        self.FC1 = nn.Conv3d(features*6, features*3, kernel_size=1, stride=1)
        self.bn1 = normalization3D(features*3, "bn")
        self.FC1_ = nn.Conv3d(features*3, out_channels, kernel_size=1, stride=1)

        self.FC2 = nn.Conv3d(features*7, features*3, kernel_size=1, stride=1)
        self.bn2 = normalization3D(features*3, "bn")
        self.DC2 = nn.ConvTranspose3d(features*3, features*3, 3, padding=1, stride=1)
        self.bn2d = normalization3D(features*3, "bn")
        self.FC2_ = nn.Conv3d(features*3, out_channels, kernel_size=1, stride=1)

        self.FC3 = nn.Conv3d(features*8, features*4, kernel_size=1, stride=1)
        self.bn3 = normalization3D(features*4, "bn")
        self.DC3 = nn.ConvTranspose3d(features*4, features*4, 1, stride=1)
        self.bn3d = normalization3D(features*4, "bn")
        self.DC3_ = nn.ConvTranspose3d(features*4, features*3, 3, padding=1, stride=1)
        self.bn3d_ = normalization3D(features*3, "bn")
        self.FC3_ = nn.Conv3d(features*3, out_channels, kernel_size=1, stride=1)

        self.FC4 = nn.Conv3d(features*8, features*4, kernel_size=1, stride=1)
        self.bn4 = normalization3D(features*4, "bn")
        self.DC4 = nn.ConvTranspose3d(features*4, features*4, 3, padding=1, stride=1)
        self.bn4d = normalization3D(features*4, "bn")
        self.DC4_ = nn.ConvTranspose3d(features*4, features*4, 3, padding=1, stride=1)
        self.bn4d_ = normalization3D(features*4, "bn")
        self.DC4__ = nn.ConvTranspose3d(features*4, features*3, 3, padding=1, stride=1)
        self.bn4d__ = normalization3D(features*3, "bn")
        self.FC4_ = nn.Conv3d(features*3, out_channels, kernel_size=1, stride=1)
...
class FCDenseNet3D_isles(nn.Module):
    def __init__(self, in_channels=1, init_features=4, out_channels=1):
        super(FCDenseNet3D_isles, self).__init__()
        self.n_features = init_features
        features = init_features

        self.conLayer1 = nn.Sequential()
        self.conLayer1.add_module("conv",
                                  nn.Conv3d(in_channels, features*2, 3, stride=(2, 2, 2), padding=1, bias=True))
        self.conLayer1.add_module("norm", normalization3D(features*2, "bn"))
        self.conLayer1.add_module("relu", nn.LeakyReLU(inplace=True))
        ...
        self.conLayer2 = nn.Sequential()
        self.conLayer2.add_module("conv",
                                  nn.Conv3d(features*6, features*3, 3, stride=(2, 2, 2), padding=1, bias=True))
        self.conLayer2.add_module("norm", normalization3D(features*3, "bn"))
        self.conLayer2.add_module("relu", nn.LeakyReLU(inplace=True))
        ...
        self.conLayer3 = nn.Sequential()
        self.conLayer3.add_module("conv",
                                  nn.Conv3d(features*7, features*4, 3, stride=(2, 2, 2), padding=1, bias=True))
        self.conLayer3.add_module("norm", normalization3D(features*4, "bn"))
        self.conLayer3.add_module("relu", nn.LeakyReLU(inplace=True))
    ...
    def forward(self, input):
        ...
        dense1 = self.DenseNet3D1(conv1)
        pre1 = self.FC1_(self.LeakyReLU(self.bn1(self.FC1(dense1))))
        pre1 = F.interpolate(pre1, scale_factor=(2, 2, 2), mode='trilinear', align_corners=True)

        conv2 = self.conLayer2(dense1)
        dense2 = self.DenseNet3D2(conv2)
        pre2 = self.LeakyReLU(self.bn2d(self.DC2(self.LeakyReLU(self.bn2(self.FC2(dense2))))))
        pre2 = F.interpolate(pre2, scale_factor=(2, 2, 2), mode='trilinear', align_corners=True)
        pre2 = self.FC2_(pre2)
        pre2 = F.interpolate(pre2, scale_factor=(2, 2, 2), mode='trilinear', align_corners=True)
        ...
        dense3 = self.DenseNet3D3(conv3)
        pre3 = self.LeakyReLU(self.bn3d(self.DC3(self.LeakyReLU(self.bn3(self.FC3(dense3))))))
        pre3 = F.interpolate(pre3, scale_factor=(2, 2, 2), mode='trilinear', align_corners=True)
        pre3 = F.interpolate(self.LeakyReLU(self.bn3d_(self.DC3_(pre3))), scale_factor=(2, 2, 2), mode='trilinear',
                             align_corners=True)
        pre3 = self.FC3_(pre3)
        pre3 = F.interpolate(pre3, scale_factor=(2, 2, 2), mode='trilinear', align_corners=True)
        ...
        dense4 = self.DenseNet3D4(conv4)
        pre4 = self.LeakyReLU(self.bn4d(self.DC4(self.LeakyReLU(self.bn4(self.FC4(dense4))))))
        pre4 = F.interpolate(pre4, scale_factor=(2, 2, 2), mode='trilinear', align_corners=True)
        pre4 = F.interpolate(self.LeakyReLU(self.bn4d_(self.DC4_(pre4))), scale_factor=(2, 2, 2), mode='trilinear',
                             align_corners=True)
        pre4 = F.interpolate(self.LeakyReLU(self.bn4d__(self.DC4__(pre4))), scale_factor=(2, 2, 2), mode='trilinear',
                             align_corners=True)
        pre4 = self.FC4_(pre4)
        pre4 = F.interpolate(pre4, scale_factor=(1, 2, 2), mode='trilinear', align_corners=True)
