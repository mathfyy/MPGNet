...
class UNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, init_features=4):
        ...
        self.encoder1 = nn.Conv3d(in_channels, 32, 3, stride=1, padding=1)  # b, 16, 10, 10
        ...
        self.decoder5 = nn.Conv3d(32, out_channels, 3, stride=1, padding=1)
        self.relu = nn.Sigmoid()

    def forward(self, input):
        out = F.relu(F.max_pool3d(self.encoder1(input), 2, 2))
        t1 = out
        ...
        output = self.relu(F.interpolate(self.decoder5(out), scale_factor=(2, 2, 2), mode='trilinear'))
