...
class ContrastLRLoss(nn.Module):
    """Supervised Contrastive Learning: https://arxiv.org/pdf/2004.11362.pdf."""

    def __init__(self, temperature=1, base_temperature=1):
        super(ContrastLRLoss, self).__init__()
        self.temperature = temperature
        self.base_temperature = base_temperature

    def forward(self, src_feature, tgt_feature, label):
        """
        both input should be l2 normalized
        Args:
            src_feature: (mb, dim)
        ...
        labels = torch.randperm(batch_size).float().to(device)
        ...
        a = ((label - torch.flip(label, [4])).sum([1, 2, 3, 4]) <= 0) * torch.ones(batch_size).float().to(device)
        log_prob = a * (logits * (1 - logits_mask)) - torch.log(exp_logits)
        # log_prob = a * logits - torch.log(exp_logits)
        # log_prob = a * (logits * (1 - logits_mask))
